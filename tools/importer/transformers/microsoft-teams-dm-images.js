...
/**
 * Transformer: microsoft-teams Dynamic Media / Scene7 images.
 * Source uses Scene7 IS/Image URLs (cdn-dynmedia-1.microsoft.com/is/image/...).
 * Converts DM <img> tags into anchors so the DM URL round-trips through
 * markdown; a companion auto-block in scripts/scripts.js rebuilds responsive
 * <picture> markup at render time. Runs in afterTransform only — block parsers
 * run between the hooks and need the <img> to build their cells.
 *
 * Helpers below are copied verbatim from the canonical dm-scene7-helpers.js
 * (the subset the transformer consumes) and must stay byte-identical.
 */
